...
use Seaf\App\Component\Request\RequestIF;
 ...
 * ルータコンポーネント
 */
class Router implements EnvironmentAcceptableIF
{
    /**
     * ルータオブジェクトたち
     * @var array
    ...
    public function map( $pattern, $callback = null )
    {
        ...
        if (strpos($pattern, ' ') !== false) 
        {
            ...
            array_push($this->routes, new Route($url, $callback, $methods, $this->environment));
        }
        else
        {
            array_push($this->routes, new Route($pattern, $callback, array('*'), $this->environment));
        }
    }
    ...
    public function route( RequestIF $request )
    {
        ...
        foreach( $this->mount as $path=>$app )
        {
            if( strpos($url,$path) === 0 ){
                ...
                return $app->router()->route( $app->request() );
            }
        }
...
class Route
{
     ...
     * @param array
     */
    public function __construct( $pattern, $callback, $methods, Environment $env ) 
    {
